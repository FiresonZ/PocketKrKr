...
| `[FontLayoutProbe]` | Compares requested and effective font heights and records the font and height used for text measurement to verify that script variables reach the native font interface intact | Font-height and text-measurement interfaces in `cpp/core/visual/LayerIntf.cpp` | Deduplicated by layer, font, height, and result; low cost |
