...
| `[TextLayerProbe]` | Record the owning layer name, rectangle, image offset, clip rectangle, and bitmap size to map local text coordinates to composited layer coordinates | `cpp/core/visual/LayerIntf.cpp`, `tTJSNI_BaseLayer::DrawText()` | Deduplicated by character, layer rectangle, image offset, and clip rectangle; low cost |
...
| `[PreRenderFontStretchProbe]` | Record the target rectangle, source rectangle, source/destination bitmap sizes, and ClipRect at the Layer prerendered-text stretch call, allowing direct comparison with the TJS parameters | `cpp/core/visual/LayerIntf.cpp`, `tTJSNI_BaseLayer::OperateStretch()` | Deduplicated by complete rectangles, layer name, and source size; low cost |
