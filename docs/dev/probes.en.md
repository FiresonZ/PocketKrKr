...
| `[TextLayerProbe]` | Record the owning layer rectangle, image offset, clip rectangle, and bitmap size to map local text coordinates to composited layer coordinates | `cpp/core/visual/LayerIntf.cpp`, `tTJSNI_BaseLayer::DrawText()` | Deduplicated by character, layer rectangle, image offset, and clip rectangle; low cost |
