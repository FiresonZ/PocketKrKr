...
| `[TextLayerProbe]` | 记录文字所属图层名称、字体、有效字号、图层矩形、图像偏移、裁剪矩形和位图尺寸，把局部文字坐标还原到图层合成坐标 | `cpp/core/visual/LayerIntf.cpp` 的 `tTJSNI_BaseLayer::DrawText()` | 按字符、图层矩形、图像偏移和裁剪矩形去重；低成本 |
| `[FontLayoutProbe]` | 对比字体高度请求值与生效值，记录文字测量实际使用的字体和字号，并在原生 drawText 修正顶部越界时记录请求与生效的 y 坐标，确认脚本变量和边界处理是否正确 | `cpp/core/visual/LayerIntf.cpp` 的字体高度、文字测量和 drawText 接口 | 按图层、字体、字号和结果去重；低成本 |
